...
namespace WEBPC.Controllers
{
    public class WEBPCController : Controller
    {
        ...
        public ActionResult sanphamhot()
        {
            ...
            var ct = db.chitietdonhangs.OrderByDescending(n => n.soluong).GroupBy(n => n.masp).Select(g=> new { masp=int.Parse(g.Key.ToString()) }).Take(1).ToList();
            var sp = new sanpham();
            foreach(var item in ct)
            {
                 sp = db.sanphams.Where(n => n.masp == item.masp).SingleOrDefault();
